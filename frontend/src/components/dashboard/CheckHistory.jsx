...
// Helper for short time display in rows
      ...
      <div className="ch-container">
        ...
        <div className="ch-summary-bar">
          ...
          <div className="ch-summary-card">
            ...
            <div className="ch-summary-value" style={{ color: "var(--success-600)" }}>
              {fmt(summary.completed)}
            </div>
          </div>
          <div className="ch-summary-card">
            <div className="ch-summary-label">ปฏิเสธ</div>
            <div className="ch-summary-value" style={{ color: "var(--danger-500)" }}>
              {fmt(summary.rejected)}
            </div>
          </div>
        </div>
        ...
        <div className="ch-filter-bar">
          <div className="ch-type-tabs">
            {Object.entries(TYPE_LABELS).map(([key, label]) => (
              <button
                key={key}
                className={`ch-type-tab${typeFilter === key ? " active" : ""}`}
                onClick={() => setTypeFilter(key)}
              >
                {key !== "all" && TYPE_ICONS[key]} {label}
                {key !== "all" && summary.byType?.[key] > 0 && (
                  <span className="badge">{summary.byType[key]}</span>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="ch-preset-group">
            <button
              className={`ch-preset-btn${presetFilter === "today" ? " active" : ""}`}
              onClick={() => handleApplyPreset("today")}
            >
              วันนี้
            </button>
            <button
              className={`ch-preset-btn${presetFilter === "month" ? " active" : ""}`}
              onClick={() => handleApplyPreset("month")}
            >
              เดือนนี้
            </button>
            <button
              className={`ch-preset-btn${presetFilter === "year" ? " active" : ""}`}
                ...
                <div className="ch-row-detail" style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div className="ch-row-text">
                      {item.text || (item.giftItems?.length ? `${item.giftItems.length} รายการ` : "—")}
                    </div>
                  </div>
                </div>
          ...
          {!loading && pagination.totalPages > 1 && (
            <div className="ch-pagination">
              ...
              <span className="ch-page-info">
                หน้า {page} / {pagination.totalPages} ({pagination.total} รายการ)
              </span>
              <button className="ch-page-btn" disabled={page >= pagination.totalPages} onClick={() => setPage((p) => p + 1)}>
                ถัดไป →
              </button>
            </div>
          )}
          ...
          <div className="ch-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ch-modal-body">
              ...
              <div className="ch-modal-field">
                ...
                <span
                  style={{
                    color: selected.price > 0 ? "var(--success-600)" : "var(--gray-400)",
                    fontWeight: 700,
                  }}
                >
              ...
              {selected.social && selected.social.type && (
                <div className="ch-modal-field">
                  <b>Social</b>
                  {selected.social.type} ({selected.social.name})
                </div>
              )}
              ...
              <div className="ch-modal-timeline">
                {selected.createdAt && (
                  <div className="ch-modal-timeline-item">
                    <div className="dot" />
                    <span>รับข้อมูล: {formatDate(selected.createdAt)}</span>
                  </div>
                )}
                {selected.checkedAt && (
                  <div className="ch-modal-timeline-item">
                    <div className="dot" />
                    <span>ตรวจสอบ: {formatDate(selected.checkedAt)}</span>
                  </div>
                )}
                {selected.startedAt && (
                  <div className="ch-modal-timeline-item">
                    <div className="dot" />
                    <span>เริ่มแสดง: {formatDate(selected.startedAt)}</span>
                  </div>
                )}
                {selected.endedAt && (
                  <div className="ch-modal-timeline-item">
                    <div className="dot" />
                    <span>จบการแสดง: {formatDate(selected.endedAt)}</span>
                  </div>
                )}
                {selected.duration && (
                  <div className="ch-modal-timeline-item">
                    <div className="dot" />
                    <span>ระยะเวลา: {selected.duration} วินาที</span>
                  </div>
                )}
              </div>
            </div>
